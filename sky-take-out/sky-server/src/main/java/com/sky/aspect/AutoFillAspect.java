 ...
 * 切⼊点
 */
// Mapper层下所有的⽅法以及加了@AutoFill注解的⽅法
...
/////////////////////重要////////////////////////////////////
//可先进⾏调试，是否能进⼊该⽅法 提前在mapper⽅法添加AutoFill注解
        log.info("开始进⾏公共字段⾃动填充...");
//获取到当前被拦截的⽅法上的数据库操作类型
        MethodSignature signature = (MethodSignature)
                joinPoint.getSignature();//⽅法签名对象
        AutoFill autoFill =
                signature.getMethod().getAnnotation(AutoFill.class);//获得⽅法上的注解对象
...
// 所有保存、更新⽅法传递的都是⼀个Entity对象，所以只需取出第⼀个便是该实体类
...
//为4个公共字段赋值
// getDeclaredMethod⽅法第⼀个参数为要获取的⽅法名，第⼆个参数为该⽅法的参数
            try {
                ...
                Method setUpdateUser =
                        entity.getClass().getDeclaredMethod(AutoFillConstant.SET_UPDATE_USER,
                                Long.class);
//通过反射为对象属性赋值，反射这⾥可以理解成，⽅法调⽤对象，跟平时反着来。
